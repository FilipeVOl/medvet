...
export default function TermoConsulta() {
  ...
  const handleAnimalSelect = (animal) => {
    // Limpar dados específicos de qualquer consulta anterior quando selecionar um novo animal
    ...
    const normalizedAnimal = {
      ...
      tutorData: {
        address: animal.tutor_address || animal.endereco || '',
        phone: animal.tutor_phone || animal.telefone || '',
        cpf: animal.tutor_cpf || animal.cpf || ''
      }
    };
  ...
  return (
    <div className="font-Montserrat pl-14">
      <h1 className="p-14 h-10 text-2xl font-bold">Termo de Consulta</h1>
      ...
      {!selectedAnimal ? (
        <div className="bg-white p-6 rounded-lg shadow-md mx-14">
          ...
          <div className="mb-6">
            <Autocomplete
              ...
              disableClearable
              id="search-animal"
              options={searchResults.map((animal) => animal.animal_name || animal.name)}
              renderInput={(params) => (
                <TextField
                  {...params}
              ...
              onChange={(_e, value) => {
                const animal = searchResults.find(a => (a.animal_name || a.name) === value);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
            {animals.map((animal) => (
              ...
              >
                ...
                <p className="text-sm text-gray-600">Espécie: {animal.species || animal.especie || 'N/A'}</p>
                <p className="text-sm text-gray-600">Raça: {animal.breed || animal.race || 'N/A'}</p>
        ...
        <div className="px-14">
          <button
            onClick={() => {
              // Limpar apenas os dados relacionados ao formulário atual
              localStorage.removeItem('motivoConsulta');
              localStorage.removeItem('observacoes');
          ...
          <FormConsulta animalData={selectedAnimal} />
        </div>
      )}
    </div>
  );
